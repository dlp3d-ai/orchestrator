...
class GeminiConversationClient(ConversationAdapter):
    """Gemini conversation client for streaming chat and reject operations.

    This client provides streaming conversation capabilities using Google
    Gemini models, with support for bracket content filtering and downstream
    task processing.
    """

    AVAILABLE_FOR_STREAM = True
    AVAILABLE_FOR_REJECT = True
    ExecutorRegistry.register_class("GeminiConversationClient")
    ...
    def __init__(
        ...
        """Initialize the Gemini conversation client.
    ...
    async def _init_llm_client(self, request_id: str) -> None:
        ...
        """
        gemini_api_key = self.input_buffer[request_id]["chat_task"].get("api_keys", {}).get("gemini_api_key", "")
    ...
    async def _llm_stream_chat(
        ...
        try:
            ...
            chat_rsp_stream = await llm_client.chat.completions.create(
                model=model_name_override if model_name_override else self.gemini_model_name,
                messages=[
                    {
                        "role": "system",
