...
class SenseNovaConversationClient(ConversationAdapter):
    ...
    def __init__(
        ...
        """Initialize the SenseNova conversation client.
        ...
        Args:
            ...
            sensenova_model_name (str, optional):
                The name of the SenseNova model to use.
                Defaults to "SenseNova-V6-5-Pro".
            sensenova_url (str, optional):
                The SenseNova API URL.
    ...
    def _gen_token(self, task_space: dict) -> str:
        """Generate a JWT token for the SenseNova API.
    ...
    async def _llm_stream_chat(
        ...
        try:
            ...
            system_content = conversation_prompt.replace(
                    ...
                    messages.append(
                        {
                            ...
                            "content": [
                                {"type": "text", "text": hist_msg.get("content", "")},
                            ],
                        }
                    )
                else:
            ...
            async with self.http_client.stream("POST", self.sensenova_url, headers=headers, json=data) as response:
                ...
                async for line in response.aiter_lines():
                    ...
                    try:
                        ...
                        self.logger.warning(f"SenseNova error processing chunk: {e}")
                        continue
                ...
                async for line in response.aiter_lines():
                    ...
                    try:
                        ...
                        if line_stripped and not line_stripped.startswith("[DONE]"):
                            self.logger.debug(
                                f"SenseNova unable to parse JSON message (may be incomplete): {line_stripped[:200]}, error: {e}"
                            )
                        continue
                    except Exception as e:
                        self.logger.warning(f"SenseNova error processing chunk: {e}, line: {line_stripped[:200]}")
                        continue
