...
class SenseChatMemoryClient(BaseMemoryAdapter):
    """SenseChat memory client that implements memory management based on
    SenseChat API.

    This class provides memory management functionality using the SenseChat API
    for LLM calls and memory operations.
    """

    ExecutorRegistry.register_class("SenseChatMemoryClient")
    ...
    def __init__(
        ...
        """Initialize the SenseChat memory client.
        ...
        Args:
            ...
            sensechat_model_name (str, optional):
                Default SenseChat model name to use. Defaults to "SenseChat-5-1202".
            sensechat_url (str, optional):
                SenseChat API URL. Defaults to "https://api.sensenova.cn/v1/llm/chat-completions".
    ...
    def _gen_token(self, api_keys: dict) -> str:
        """Generate a JWT token for the SenseChat API authentication.
    ...
    async def call_llm(
        ...
        """Call SenseChat LLM for text generation.
        ...
        Returns:
            str:
                Generated text content from the SenseChat LLM.
        """
        try:
            if not api_keys:
                raise ValueError("api_keys is required for SenseChat LLM calls")
            sensechat_ak = api_keys.get("sensechat_ak", "")
            sensechat_sk = api_keys.get("sensechat_sk", "")
            ...
            sensechat_model_name = model_override if model_override else self.sensechat_model_name
