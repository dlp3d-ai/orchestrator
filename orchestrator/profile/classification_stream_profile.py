from ..data_structures.classification import ClassificationChunkBody, ClassificationChunkEnd, ClassificationChunkStart
...
class ClassificationStreamProfile(StreamProfile):
    """Classification stream profile."""

    def __init__(self, *args, **kwargs):
        """Initialize the classification stream profile.
    ...
    async def _handle_start(self, chunk: ClassificationChunkStart, cur_time: float) -> None:
        ...
        Args:
            chunk (ClassificationChunkStart):
                The classification start chunk containing request information.
            cur_time (float):
                Current timestamp.
        """
    ...
    async def _handle_body(self, chunk: ClassificationChunkBody, cur_time: float) -> None:
        ...
        Args:
            chunk (ClassificationChunkBody):
                The classification body chunk containing classification results.
            cur_time (float):
                Current timestamp.
        """
        msg = (
            f"Received body chunk for request {chunk.request_id}, classification result: {chunk.classification_result}."
        )
        self.logger.info(msg)
    ...
    async def _handle_end(self, chunk: ClassificationChunkEnd, cur_time: float) -> None:
        ...
        Args:
            chunk (ClassificationChunkEnd):
                The classification end chunk signaling completion.
            cur_time (float):
                Current timestamp.
        """
