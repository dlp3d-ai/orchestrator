...
class XAIReactionClient(ReactionAdapter):
    """Reaction client using xAI's API for emotion and motion analysis.

    This client uses xAI's Grok language models to analyze conversation context
    ...
    async def get_reaction_delta(
        ...
        """Get the reaction delta according to user's text input using XAI LLM.
        ...
        Raises:
            Exception:
                If the XAI API request fails or response parsing fails.
        """
        ...
        try:
            ...
            self.logger.debug(f"xai spent {time.time() - start_time} seconds to get reaction delta: {response_delta}")
