...
def setup_logger(
    ...
    Create a logger with specified configuration, supporting simultaneous output to console, file and AWS CloudWatch,
    with separate log levels for different output streams.
    ...
    Args:
        ...
        logger_level (int | None, optional):
            Global log level for logger, deprecated, please use file_level/console_level,
            defaults to None.
        logger_path (str | None, optional):
            Log file path, when None no file writing, defaults to None.
        aws_group_name (str | None, optional):
            AWS CloudWatch log group name, when None no AWS output, defaults to None.
        aws_stream_name (str | None, optional):
            AWS CloudWatch log stream name, when None no AWS output, defaults to None.
        aws_access_key_id (str | None, optional):
            AWS access key ID, defaults to None.
        aws_secret_access_key (str | None, optional):
            AWS access key, defaults to None.
        aws_region (str | None, optional):
            AWS region name, defaults to None.
        logger_format (str | None, optional):
            Log format string, when None uses default format, defaults to None.
        aws_send_interval (int, optional):
            AWS CloudWatch log send interval (seconds), recommended 5-30 seconds to reduce API calls,
            defaults to 5 seconds.
        aws_use_queues (bool, optional):
            Whether to use queues for async AWS CloudWatch log sending, True improves performance but may
            generate warnings on program exit, defaults to True.
    ...
    Note:
        logger_level parameter is deprecated, recommend using file_level, console_level and aws_level
        to set log levels for different output streams separately.
        Program will automatically gracefully close all CloudWatch log handlers on exit to avoid warnings.
    """
    ...
    if aws_group_name is not None and aws_stream_name is not None:
        aws_logger_key = f"{aws_group_name}-{aws_stream_name}-{aws_access_key_id}-{aws_region}-{aws_level}"
    ...
    if logger_level is not None:
        logger.warning(
            "UserWarning: logger_level is now deprecated, " + "please specify file_level/console_level instead."
        )
    return logger
...
def get_logger(logger: None | str | logging.Logger = None) -> logging.Logger:
    ...
    Get corresponding logger instance based on input parameters, supports multiple input types.

    Args:
        logger (None | str | logging.Logger, optional):
            Logger identifier, None means get root logger, string means logger name,
            defaults to None.
...
def shutdown_cloudwatch_handlers():
    ...
    avoid "Received message after logging system shutdown" warnings on program exit.
    This function will be automatically called on program exit.

    Note:
        This function is registered through atexit module and automatically executes on normal program exit.
        If exceptions occur during shutdown, they will be silently ignored to ensure normal program exit.
    """
    try:
